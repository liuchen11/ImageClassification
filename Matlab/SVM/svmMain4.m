...
% Construct 6 SVM classifiers

SVMModels = cell(1,6);
rng(1); 

for j = 1:6  
    SVMModels{1,j} = fitcsvm(trainCell{1,j}(:,1:end-1),trainCell{1,j}(:,end),'KernelFunction','linear','BoxConstraint',5e0);
end 
                                                                            % change type of kernel function
                                                                            % change C by tuning BoxConstraint
label = [];

% making label predictions using constructed models
for j = 1:6 
    [label(:,j),~] = predict(SVMModels{j},Xte(:,:));
end
...
out(k) = BER;
pred = zeros(1,length(pred)); % reinitialize the array
end
